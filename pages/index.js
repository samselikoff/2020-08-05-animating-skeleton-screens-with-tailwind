...
import { useBufferedData } from "../lib/hooks/useBufferedData";
...
function Home() {
  ...
  useEffect(() => {
    window.toggle = () => setToggle(!toggle);
  });
      ...
      <div className="divide-y divide-gray-200">
        {!data || toggle ? (
          ...
          [...data?.tweets].reverse().map((tweet) => (
            <div className="relative px-4 py-2" key={tweet.id}>
              <Tweet tweet={tweet} />
            </div>
          ))
        )}
...
function SkeletonTweet({ type = "short" }) {
  return (
    <div className="flex">
...
function Tweet({ tweet }) {
  return (
    <div className="flex">
      <img
        className="flex-shrink-0 w-12 h-12 mr-3 bg-gray-200 rounded-full"
        src={tweet.avatarUrl}
      />
